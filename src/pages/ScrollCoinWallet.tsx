...
const ScrollCoinWallet: React.FC = () => {
  ...
  const { data: walletData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Get scrollcoin transactions for detailed history
      const { data: scTransactions, error: scError } = await supabase
  ...
  if (!user) {
    return (
      <div className="container mx-auto px-4 py-8">
        <Alert>
          <AlertDescription>Please log in to access your ScrollCoin wallet.</AlertDescription>
        </Alert>
      </div>
    );
  }
      ...
      <Card className="mb-8 p-6 bg-gradient-to-br from-primary/10 to-primary/5">
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <div>
            ...
            <p className="text-3xl font-bold text-primary">{(wallet?.balance || 0).toFixed(2)} SC</p>
          ...
          <div>
            ...
            <p className="text-2xl font-semibold text-green-600">{(wallet?.totalEarned || 0).toFixed(2)} SC</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground mb-1">Total Spent</p>
            <p className="text-2xl font-semibold text-orange-600">{(wallet?.totalSpent || 0).toFixed(2)} SC</p>
          </div>
          <div>
            <p className="text-sm text-muted-foreground mb-1">Net Rewards</p>
            <p className="text-2xl font-semibold text-blue-600">{(wallet?.netRewards || 0).toFixed(2)} SC</p>
          </div>
        </div>
      </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-2xl font-bold text-green-600">
                      +{wallet.statistics.weeklyEarnings.toFixed(2)} SC
                    </p>
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-2xl font-bold text-orange-600">
                      -{wallet.statistics.weeklySpending.toFixed(2)} SC
                    </p>
                    <p className="text-xs text-muted-foreground mt-1">Last 7 days</p>
                  </CardContent>
                </Card>
                ...
                <Card>
                  ...
                  <CardContent>
                    <p className="text-2xl font-bold">
                      {wallet.statistics.averageTransactionAmount.toFixed(2)} SC
                    </p>
        ...
        <TabsContent value="transactions">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your recent ScrollCoin transactions</CardDescription>
            </CardHeader>
            <CardContent>
              {transactions.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">Complete courses and activities to earn ScrollCoins!</p>
                </div>
              ) : (
                  ...
                  {transactions.map((tx: any) => (
                    ...
                    >
                      ...
                      <p className={`font-bold ${Number(tx.amount) > 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {Number(tx.amount) > 0 ? '+' : ''}{Number(tx.amount).toFixed(2)} SC
                      </p>
                    </div>
                  ))}
                </div>
        ...
        <TabsContent value="earn">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Ways to earn more ScrollCoins</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  { title: 'Complete a Course Module', reward: '+10 SC', description: 'Finish any course module', icon: '📚' },
                  { title: 'Pass a Quiz', reward: '+5 SC', description: 'Score 70% or higher on a quiz', icon: '✅' },
                  { title: 'Daily Study Streak', reward: '+1 SC/day', description: 'Study every day to earn bonus coins', icon: '🔥' },
                  { title: 'Help a Fellow Student', reward: '+15 SC', description: 'Answer questions in the community', icon: '🤝' },
                  { title: 'Complete Daily Devotion', reward: '+3 SC', description: 'Read and reflect on daily devotions', icon: '🙏' },
                  { title: 'Scripture Memory', reward: '+5 SC', description: 'Memorize a new scripture verse', icon: '📖' },
                ].map((opportunity, idx) => (
                  <Card key={idx} className="border-2 hover:border-primary transition-colors cursor-pointer">
                    <CardContent className="pt-6">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="text-green-600 font-bold">{opportunity.reward}</span>
                      </div>
                    </CardContent>
                  </Card>
                ))}
        ...
        <TabsContent value="spend">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Use your ScrollCoins for these benefits</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid gap-4 md:grid-cols-2">
                {[
                  { title: 'Premium Course Access', cost: '100 SC', description: 'Unlock a premium course', icon: '🎓' },
                  { title: 'Extended AI Tutoring', cost: '50 SC', description: '1 hour of extra AI tutor time', icon: '🤖' },
                  { title: 'Certificate Upgrade', cost: '200 SC', description: 'Add blockchain verification to your certificate', icon: '📜' },
                  { title: 'Mentorship Session', cost: '75 SC', description: 'Book a 1-on-1 mentorship session', icon: '👨‍🏫' },
                  { title: 'Study Resources Pack', cost: '25 SC', description: 'Download premium study materials', icon: '📁' },
                  ...
                  <Card key={idx} className="border-2 hover:border-primary transition-colors cursor-pointer">
                    <CardContent className="pt-6">
                      <div className="flex items-start justify-between">
                        ...
                        <span className="text-orange-600 font-bold">{option.cost}</span>
                      </div>
                    </CardContent>
                  </Card>
                ))}
