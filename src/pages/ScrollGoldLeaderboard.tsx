...
import { useScrollGoldLeaderboard } from "@/hooks/useScrollGoldTransactions";

console.info("✝️ ScrollGold Leaderboard — Faithful stewards honored");

export default function ScrollGoldLeaderboard() {
  const { data: leaderboard, isLoading } = useScrollGoldLeaderboard(50);
  ...
  const getRankBadge = (index: number) => {
    if (index === 0) return <Badge className="bg-amber-500">Gold</Badge>;
    ...
    >
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Students with the most ScrollGold earned through faithful service
          </CardDescription>
        </CardHeader>
        <CardContent>
