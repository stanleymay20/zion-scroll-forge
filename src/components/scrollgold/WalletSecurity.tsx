...
import { WalletSecuritySettings } from '@/types/scrollgold';
...
const WalletSecurity: React.FC<WalletSecurityProps> = ({ walletAddress }) => {
  ...
  const loadSecuritySettings = async () => {
    try {
      ...
      // const response = await fetch('/api/scrollgold/wallet/security');
  ...
  const verifyTransaction = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/scrollgold/blockchain/verify', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
