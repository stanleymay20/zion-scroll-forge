...
const capabilities = [
  ...
  },
  {
    ...
    desc: "Earn SG for completing modules, passing assessments, and serving the community.",
      ...
      <div className="container mx-auto max-w-6xl relative">
        <div className="text-center mb-14 sm:mb-20">
          ...
          <p className="text-sm sm:text-base text-muted-foreground font-sans max-w-2xl mx-auto leading-relaxed">
            Not an LMS. A fully governed university with verifiable credentials, AI faculty,
            spiritual formation, and a token economy—engineered for the kingdom.
          </p>
        </div>
                ...
                <div className="relative h-full bg-card rounded-2xl border border-border/60 p-6 card-hover overflow-hidden">
                  <div
                    className={`absolute -top-12 -right-12 w-32 h-32 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 ${
                      isAccent ? "bg-accent/20" : "bg-primary/15"
                    }`}
                  />
                  <div className="relative">
                    ...
                    >
                      <Icon
                        className={`w-5 h-5 ${isAccent ? "text-accent" : "text-primary"}`}
                      />
                    </div>
                    <h3 className="font-serif font-semibold text-base text-primary mb-2 leading-snug">
                      {cap.title}
                    </h3>
