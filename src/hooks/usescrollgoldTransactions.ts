...
export async function awardScrollGolds(params: {
  amount: number;
  source: string;
  related_id?: string;
  description?: string;
...
export async function spendScrollGolds(params: {
  amount: number;
  source: string;
  related_id?: string;
  description?: string;
...
export const useAwardScrollGolds = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: awardScrollGolds,
    onSuccess: () => {
      toast({ title: "✝️ ScrollGolds awarded — God rewards faithfulness!" });
      qc.invalidateQueries({ queryKey: ["ScrollGold-transactions"] });
      qc.invalidateQueries({ queryKey: ["ScrollGold-balance"] });
    },
    onError: (e: any) => toast({
      title: "Failed to award ScrollGolds",
...
export const useSpendScrollGolds = () => {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: spendScrollGolds,
    onSuccess: () => {
      toast({ title: "✅ ScrollGolds spent" });
      qc.invalidateQueries({ queryKey: ["ScrollGold-transactions"] });
      qc.invalidateQueries({ queryKey: ["ScrollGold-balance"] });
    },
    onError: (e: any) => toast({
      title: "Failed to spend ScrollGolds",
      description: e.message,
      variant: "destructive"
    })
  });
