...
-- 3️⃣ ScrollGold On-Chain Bridge Preparation
...
-- Bridge audit log
create table if not exists ScrollGold_bridge_log (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null,
  tx_hash text,
...
-- Enable RLS
alter table ScrollGold_bridge_log enable row level security;

create policy "Users can view own bridge logs"
  on ScrollGold_bridge_log for select
  using (auth.uid() = user_id);
...
select
  ...
  (select sum(amount) from transactions where type='earned') as total_ScrollGold_earned,
  (select sum(amount) from transactions where type='spent') as total_ScrollGold_spent,
...
-- Note: Email is in auth.users, not profiles. This update is skipped for now.
-- Faculty roles should be assigned through the admin interface or during user creation.
