...
-- Ensure scrollgolds column exists
alter table profiles add column if not exists scrollgolds int default 0;
...
create or replace function award_scrollgolds(p_user uuid, p_event text, p_amount int, p_meta jsonb default '{}'::jsonb)
returns void
language plpgsql
security definer
as $$
begin
  ...
  update profiles set scrollgolds = coalesce(scrollgolds,0) + p_amount where id = p_user;
end;
$$;
...
begin
  ...
  if not found then
    perform award_scrollgolds(p_user, p_event, 1, p_meta);
    return;
  end if;
  ...
  perform award_scrollgolds(p_user, p_event, amt, p_meta);
end;
$$;
