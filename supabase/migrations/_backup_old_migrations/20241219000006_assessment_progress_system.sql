...
BEGIN
    ...
    -- Award ScrollGolds for passing
    IF percentage >= (SELECT passing_score FROM assessments WHERE id = attempt_record.assessment_id) THEN
        ScrollGolds_earned := LEAST(ROUND(percentage), 100); -- 1 coin per percentage point, max 100
