...
CREATE TABLE IF NOT EXISTS public.scrollgold_analytics_daily (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX idx_scrollgold_analytics_date ON public.scrollgold_analytics_daily(date);
...
ALTER TABLE public.scrollgold_analytics_daily ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Faculty and admin can view scrollgold analytics"
  ON public.scrollgold_analytics_daily FOR SELECT
  TO authenticated
  USING (
    EXISTS (
      SELECT 1 FROM public.profiles
