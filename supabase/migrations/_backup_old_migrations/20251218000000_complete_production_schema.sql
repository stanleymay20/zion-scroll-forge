...
DO $$ BEGIN
    CREATE TYPE payment_method AS ENUM (
        'CREDIT_CARD', 'scroll_gold', 'CRYPTOCURRENCY', 'WORK_TRADE', 'SCHOLARSHIP'
    );
EXCEPTION
    WHEN duplicate_object THEN null;
END $$;
...
-- ScrollGold Activity Enum
DO $$ BEGIN
    CREATE TYPE ScrollGold_activity AS ENUM (
...
CREATE TABLE IF NOT EXISTS public.user_profiles (
    ...
    phone_number TEXT,
    date_of_birth DATE,
    scroll_gold_wallet TEXT UNIQUE,
    ...
    scroll_gold_balance DECIMAL(10,2) DEFAULT 0.0,
...
CREATE TABLE IF NOT EXISTS public.ScrollGold_transactions (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    amount DECIMAL(10,2) NOT NULL,
    ...
    activity_type ScrollGold_activity,
    related_entity_id TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_ScrollGold_transactions_user_id ON public.ScrollGold_transactions(user_id);
CREATE INDEX IF NOT EXISTS idx_ScrollGold_transactions_type ON public.ScrollGold_transactions(type);
CREATE INDEX IF NOT EXISTS idx_ScrollGold_transactions_created_at ON public.ScrollGold_transactions(created_at);
...
ALTER TABLE public.ScrollGold_transactions ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Faculty can view course submissions" ON public.submissions;
...
DROP POLICY IF EXISTS "Users can view own transactions" ON public.ScrollGold_transactions;
CREATE POLICY "Users can view own transactions"
    ON public.ScrollGold_transactions FOR SELECT
    USING (auth.uid() = user_id);

-- System can create transactions (handled by backend)
DROP POLICY IF EXISTS "System can create transactions" ON public.ScrollGold_transactions;
CREATE POLICY "System can create transactions"
    ON public.ScrollGold_transactions FOR INSERT
    WITH CHECK (true);
...
DROP POLICY IF EXISTS "Authors can view own papers" ON public.research_papers;
CREATE POLICY "Authors can view own papers"
    ON public.research_papers FOR SELECT
    USING (auth.uid() = author_id);

-- Everyone can view published papers
DROP POLICY IF EXISTS "Anyone can view published papers" ON public.research_papers;
CREATE POLICY "Anyone can view published papers"
    ON public.research_papers FOR SELECT
    USING (status = 'PUBLISHED');

-- Authors can create papers
DROP POLICY IF EXISTS "Authors can create papers" ON public.research_papers;
...
DROP POLICY IF EXISTS "Students can view own badges" ON public.scrollbadges;
CREATE POLICY "Students can view own badges"
    ON public.scrollbadges FOR SELECT
    USING (auth.uid() = student_id);

-- Public badges are viewable by anyone
DROP POLICY IF EXISTS "Anyone can view public badges" ON public.scrollbadges;
...
DROP POLICY IF EXISTS "System can create badges" ON public.scrollbadges;
...
DROP POLICY IF EXISTS "Admissions staff can update applications" ON public.applications;
...
DROP POLICY IF EXISTS "Users can view own AI sessions" ON public.ai_tutor_sessions;
...
DROP POLICY IF EXISTS "Users can create own AI sessions" ON public.ai_tutor_sessions;
...
DROP POLICY IF EXISTS "Users can update own AI sessions" ON public.ai_tutor_sessions;
...
BEGIN
    -- Get course cost
    SELECT scroll_gold_cost INTO v_course_cost
    FROM public.courses
    ...
    -- Get user balance
    SELECT scroll_gold_balance INTO v_user_balance
    ...
    -- Deduct ScrollGold if cost > 0
    IF v_course_cost > 0 THEN
        UPDATE public.user_profiles
        SET scroll_gold_balance = scroll_gold_balance - v_course_cost
        WHERE id = p_user_id;
        
        -- Record transaction
        INSERT INTO public.ScrollGold_transactions (
...
-- Function to complete a course and award ScrollGold
...
DECLARE
    ...
    v_course_id UUID;
    v_xp_reward INTEGER;
    v_ScrollGold_reward DECIMAL(10,2);
BEGIN
    ...
    -- Calculate ScrollGold reward (1 ScrollGold per 10 XP)
    v_ScrollGold_reward := v_xp_reward / 10.0;
    ...
    -- Award ScrollGold
    UPDATE public.user_profiles
    SET scroll_gold_balance = scroll_gold_balance + v_ScrollGold_reward
    WHERE id = v_user_id;
    ...
    INSERT INTO public.ScrollGold_transactions (
        ...
        v_user_id, v_ScrollGold_reward, 'EARNED', 
...
BEGIN
    ...
    -- If payment is with ScrollGold, deduct from balance
    IF p_method = 'scroll_gold' THEN
        UPDATE public.user_profiles
        SET scroll_gold_balance = scroll_gold_balance - p_amount
        WHERE id = p_user_id;
        
        -- Record ScrollGold transaction
        INSERT INTO public.ScrollGold_transactions (
            user_id, amount, type, description
        ) VALUES (
...
DROP TRIGGER IF EXISTS update_user_profiles_updated_at ON public.user_profiles;`r`nCREATE TRIGGER update_user_profiles_updated_at`r`n    BEFORE UPDATE ON public.user_profiles
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_faculties_updated_at ON public.faculties;`r`nCREATE TRIGGER update_faculties_updated_at`r`n    BEFORE UPDATE ON public.faculties
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_courses_updated_at ON public.courses;`r`nCREATE TRIGGER update_courses_updated_at`r`n    BEFORE UPDATE ON public.courses
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_enrollments_updated_at ON public.enrollments;`r`nCREATE TRIGGER update_enrollments_updated_at`r`n    BEFORE UPDATE ON public.enrollments
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_assignments_updated_at ON public.assignments;`r`nCREATE TRIGGER update_assignments_updated_at`r`n    BEFORE UPDATE ON public.assignments
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_submissions_updated_at ON public.submissions;`r`nCREATE TRIGGER update_submissions_updated_at`r`n    BEFORE UPDATE ON public.submissions
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_research_papers_updated_at ON public.research_papers;`r`nCREATE TRIGGER update_research_papers_updated_at`r`n    BEFORE UPDATE ON public.research_papers
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_scrollbadges_updated_at ON public.scrollbadges;`r`nCREATE TRIGGER update_scrollbadges_updated_at`r`n    BEFORE UPDATE ON public.scrollbadges
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_public_badge_profiles_updated_at ON public.public_badge_profiles;`r`nCREATE TRIGGER update_public_badge_profiles_updated_at`r`n    BEFORE UPDATE ON public.public_badge_profiles
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_applications_updated_at ON public.applications;`r`nCREATE TRIGGER update_applications_updated_at`r`n    BEFORE UPDATE ON public.applications
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();

DROP TRIGGER IF EXISTS update_ai_tutor_sessions_updated_at ON public.ai_tutor_sessions;`r`nCREATE TRIGGER update_ai_tutor_sessions_updated_at`r`n    BEFORE UPDATE ON public.ai_tutor_sessions
    FOR EACH ROW
    EXECUTE FUNCTION public.update_updated_at_column();
...
CREATE TRIGGER audit_ScrollGold_transactions
    AFTER INSERT OR UPDATE OR DELETE ON public.ScrollGold_transactions
    FOR EACH ROW
    EXECUTE FUNCTION public.log_audit_trail();
...
GROUP BY up.id, up.username, up.scroll_gold_balance;
...
BEGIN
    ...
    GROUP BY c.id, c.title, c.description, c.difficulty, c.duration, c.scroll_gold_cost, f.name
...
CREATE OR REPLACE FUNCTION public.get_leaderboard(
    ...
    scroll_gold_balance DECIMAL(10,2),
...
BEGIN
    ...
    FROM public.ScrollGold_transactions
    ...
    IF v_last_activity = CURRENT_DATE - INTERVAL '1 day' THEN
        ...
        v_bonus_amount := 1.0 + (v_streak_days * 0.1); -- Base 1 ScrollGold + 0.1 per day
        
        -- Award bonus
        UPDATE public.user_profiles
        SET scroll_gold_balance = scroll_gold_balance + v_bonus_amount
        WHERE id = p_user_id;
...
ALTER PUBLICATION supabase_realtime ADD TABLE public.ScrollGold_transactions;
...
CREATE INDEX IF NOT EXISTS idx_ScrollGold_transactions_user_type ON public.ScrollGold_transactions(user_id, type);
...
BEGIN
    ...
    RAISE NOTICE 'Tables created: user_profiles, faculties, courses, enrollments, assignments, submissions, payments, ScrollGold_transactions, research_papers, certifications, scrollbadges, badge_verifications, public_badge_profiles, applications, ai_tutor_sessions, messages, audit_logs';
    ...
    RAISE NOTICE 'Realtime enabled for: messages, enrollments, submissions, ai_tutor_sessions, ScrollGold_transactions';
END $$;
