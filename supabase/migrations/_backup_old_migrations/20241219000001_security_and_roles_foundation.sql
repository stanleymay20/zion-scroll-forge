...
-- ScrollGold economy functions
CREATE OR REPLACE FUNCTION public.earn_ScrollGold(
    user_uuid UUID,
    amount INTEGER,
    reason TEXT,
...
BEGIN
    ...
    -- Get current balance
    SELECT COALESCE(ScrollGold_balance, 0) INTO current_balance
    FROM profiles WHERE id = user_uuid;
    
    -- Update balance
    UPDATE profiles 
    SET 
        ScrollGold_balance = COALESCE(ScrollGold_balance, 0) + amount,
        updated_at = NOW()
    WHERE id = user_uuid;
...
CREATE OR REPLACE FUNCTION public.spend_ScrollGold(
    user_uuid UUID,
    amount INTEGER,
    reason TEXT,
    reference_id UUID DEFAULT NULL,
...
BEGIN
    ...
    UPDATE profiles 
    SET 
        ScrollGold_balance = current_balance - amount,
        updated_at = NOW()
    WHERE id = user_uuid;
...
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS ScrollGold_balance INTEGER DEFAULT 0;
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS spiritual_metrics JSONB DEFAULT '{}';
ALTER TABLE profiles ADD COLUMN IF NOT EXISTS preferences JSONB DEFAULT '{}';

-- Create ScrollGold transactions table
...
-- RLS Policies for ScrollGold_transactions
ALTER TABLE public.ScrollGold_transactions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view their own transactions" ON public.ScrollGold_transactions
    FOR SELECT USING (user_id = auth.uid());

CREATE POLICY "System can create transactions" ON public.ScrollGold_transactions
